#### Title: Create Account Timescale dbt Model in Silver Layer

#### Description
As a Data Engineer, I want to pivot and transform account data from the TimescaleDB source into the Silver layer, So that we can compare it with Apify metrics to find and resolve data discrepancies.
...
* **Logic/Code:** Pivot account-level information from `raw_timescaledb` using dbt transformations. Implement logic to align with Apify metrics for comparison.
* **Naming/Paths:** Model name: `stg_timescaledb_social_accounts`. Snowflake Silver layer schema.
...
- [x] Code follows naming conventions (`stg_timescaledb_social_accounts`)
